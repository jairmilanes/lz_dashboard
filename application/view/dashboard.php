...
<div id="lz_dashboard" class="grid-system">
	<div class="lz_tabs grid-row grid-first-row grid-100">
		<ul>
			<?php /* <li><a href="<?php echo osc_route_admin_url('lz_dashboard/index');?>"><?php _e('Home','lz_payments');?></a></li>*/ ?>
			<?php $i =0; foreach( $data->plugins as $plugin ){ ?>
				<li class="<?php echo $plugin->shortname;?> box" data-plugin="<?php echo $plugin->shortname;?>">
					<a data-hash="<?php echo '#'.$plugin->shortname;?>" href="<?php echo osc_admin_ajax_hook_url('lz_dashboard_load_'.$plugin->shortname)?>"><?php echo $plugin->name?></a>
				</li>
			<?php } ?>
		</ul>
	</div>
